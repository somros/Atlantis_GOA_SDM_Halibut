---
title: "Adult Pacific halibut CPUE with sdmTMB - lat, lon, depth"
...
NOTE: we use bottom trawl data for halibut. The maps produced here are intended as a validation / exploration tool. Reasons are:
...
This document applies the sdmTMB routine to the IPHC FISS data filtered to focus on Pacific halibut *Hippoglossus stenolepis*. This is longline data that cannot be standardized by area like CPUE for the bottom trawl surveys could be. CPUE was calculated as weight of halibut (legal and sublegal) per effective hook for each soak. See file halibut_IPHC.Rmd for details on how the data for the models was created.

Data received from the AKFIN team (Jean Lee) on October 22 2021 - did not query this myself from IPHC.

Because of the sampling method (setlines) this is probably best used only for adults.
...
```{r}
halibut_data <- readRDS('../data/IPHC/halibut.RDS')
```
...
halibut_bbox <- halibut_data %>% st_as_sf(coords=c('lon','lat'),crs=4326) %>% st_bbox()

ggplot()+
  geom_point(data = halibut_data, aes(lon, lat, colour = log1p(cpue)), size = 1.5)+
  ...
  labs(title = "CPUE from IPHC surveys")
```

Take a quick look at time series of total CPUE from raw data
```{r}
biom_year <- halibut_data %>% group_by(year) %>% summarise(cpue = sum(cpue))

ggplot(biom_year, aes(year, log(cpue)))+
  geom_point()+
  geom_path()+
  theme_minimal()+
  labs(title = "CPUE from IPHC surveys")
...
halibut_data <- halibut_data_sf %>% 
  st_set_geometry(NULL) %>% 
  data.frame(halibut_coords)
```

Adding an `sf` object of the coastline to incorporate in the mesh.
```{r}
data_bbox <- halibut_data_sf %>% st_bbox()
...
```{r}
halibut_spde <- make_mesh(halibut_data, c("x", "y"), cutoff = 20, type = "cutoff")

# add barrier
halibut_spde <- add_barrier_mesh(
  spde_obj = halibut_spde,
...
```{r}
hist(halibut_data$cpue, breaks = 30)
```

```{r}
hist(log1p(halibut_data$cpue), breaks = 30)
```

Proportion of zeroes in percentage.
```{r}
length(which(halibut_data$cpue == 0))/nrow(halibut_data)*100
```
...
m_depth <- sdmTMB(
    data = halibut_data, 
    formula = cpue ~ 0 + s(depth, k = 3) + as.factor(year), 
    mesh = halibut_spde, 
    time = "year", 
    spatial = 'on',
...
```{r}
halibut_data$resids <- residuals(m_depth) # randomized quantile residuals
hist(halibut_data$resids)
```

And QQ plot.
```{r}
qqnorm(halibut_data$resids)
abline(a = 0, b = 1)
```
...
```{r, fig.width = 12, fig.height=18}
halibut_sf <- halibut_data %>% mutate(x = x*1000, y = y*1000) %>% st_as_sf(coords = c(x = "x", y = "y"), crs = atlantis_crs) #%>% st_transform(crs = atlantis_bgm$extra$projection) # turn to spatial object
# 
coast_sf1 <- coast_sf %>% st_transform(crs = atlantis_crs)

ggplot()+
  geom_sf(data = halibut_sf, aes(color = resids, alpha = .8))+
  scale_color_viridis()+
...
# add year column
all_years <- levels(factor(halibut_data$year))
...
```{r}
predictions_halibut <- predict(m_depth, newdata = atlantis_grid, return_tmb_object = TRUE)
atlantis_grid$estimates <- exp(predictions_halibut$data$est) #Back-transforming here
...
ggplot()+
  ...
  # geom_sf(data=coast_sf)+
  # coord_sf(xlim=c(halibut_bbox$xmin,halibut_bbox$xmax), ylim=c(halibut_bbox$ymin,halibut_bbox$ymax))+
  scale_color_viridis(name = expression(paste("Log(CPUE) lbs ", hook^-1)))+
  theme_minimal()+
  labs(title = "Predicted CPUE")+
  facet_wrap(~year, ncol = 2)
```
...
ggplot()+
  ...
  scale_fill_viridis(name = expression(paste("Log(CPUE) lbs ", hook^-1)))+
  theme_minimal()+
  #geom_sf(data = coast_sf, colour = "black", fill = "grey80")+
  facet_wrap(~year, ncol = 2)+
  labs(title = "mean predicted CPUE by Atlantis box")
```

Plot the raw data again for comparison.
```{r, fig.width = 12, fig.height = 18}
ggplot()+
  geom_sf(data = halibut_data_sf, aes(colour = log1p(cpue)), size = 1.5, alpha = .5)+
  scale_colour_viridis_c(name = expression(paste("Log(CPUE) lbs ", hook^-1)))+
  #geom_sf(data = coast_sf, colour = "black", fill = "grey80")+
  theme_minimal()+
  facet_wrap(~year, ncol = 2)+
  labs(title = "mean predicted CPUE by Atlantis box")
...
```{r}
ggplot(data = halibut_data, aes(x = depth, y = log1p(cpue)))+
  scale_color_viridis()+
  geom_point()+
  theme_minimal()+
...
```{r}
ggplot(data = halibut_data, aes(x = log1p(cpue)))+
...
means_all_years <- predictions_by_box %>% group_by(box_id, area, boundary) %>% summarise(all_years_cpue = mean(mean_estimates)) %>% ungroup()

ggplot()+
  geom_sf(data = means_all_years, aes(fill = log1p(all_years_cpue)))+ # log for visualisation
  scale_fill_viridis(name = expression(paste("Log(CPUE) lbs ", hook^-1)))+
  #geom_sf(data = coast_sf, colour = "black", fill = "grey80")+
  theme_minimal()+
  labs(title = "Mean predicted CPUE by Atlantis box (1998-2019)")
```
...
ggplot()+
  ...
  #geom_sf(data = coast_sf, colour = "black", fill = "grey80")+
  theme_minimal()+
  labs(title = "CV of predicted CPUE by Atlantis box (1998-2019)")
```
...
halibut_grid_tmp <- halibut_data %>% dplyr::select(x, y, depth)

# add year
halibut_grid <- halibut_grid_tmp[rep(1:nrow(halibut_grid_tmp), length(all_years)),]
halibut_grid$year <- as.integer(rep(all_years, each = nrow(halibut_grid_tmp)))

# predict on this grid
predictions_at_locations <- predict(m_depth, newdata = halibut_grid, return_tmb_object = TRUE)
halibut_grid$predictions <- exp(predictions_at_locations$data$est) # back-transforming here
```

Now join by year and coordinates to have predictions at the sampling points. 
```{r, fig.width = 12, fig.height = 6}
halibut_corr <- halibut_data %>% left_join(halibut_grid, by = c("year", "y", "x"))
```

## Observed versus predicted

```{r}
paste0("Pearson's coef observations vs predictions: ", cor(halibut_corr$cpue, halibut_corr$predictions, use = "everything", method = "pearson"))
```

Plot.
```{r, fig.width = 12, fig.height = 12}
ggplot(halibut_corr, aes(x = log1p(cpue), y = log1p(predictions)))+ # log for visualisation
  geom_point(aes(color = depth.y))+
  scale_color_viridis()+
  geom_abline(intercept = 0, slope = 1)+
  theme_minimal()+
  facet_wrap(~year, scales = "free")+
  labs(title = "Observed vs predicted CPUE")
```
...
halibut_data %>%
  ggplot()+
  geom_point(aes(x = depth, y = resids, color = log1p(cpue)))+
...
```{r}
paste("RMSE:", sqrt(sum((halibut_corr$predictions - halibut_corr$cpue)^2)/nrow(halibut_corr)), " lbs hook-1", sep = " ") ### traditional rmse metric, in units kg km2
```

Normalised RMSE. 
```{r}
rmse_cv <- sqrt(sum((halibut_corr$predictions - halibut_corr$cpue)^2)/nrow(halibut_corr))/(max(halibut_corr$cpue)-min(halibut_corr$cpue))*100 #### normalised rmse, expressed as a % of the range of observed biomass values, sort of approximates a coefficient of variation 
paste("Normalised RMSE:", paste0(rmse_cv, "%"), sep = " ")
```

# S1-S4

While this is dedicated data for halibut and it eliminates the issue of discontinuity with BC, for the reasons listed at the beginning of this document we still use RACE-GAP data for halibut distributions.

The problem is that these are CPUEs in fish weight per hook. Multiplying these by areas makes no sense, but to obtain an abstract abundance index that then we use to work out relative proportions per box it should be OK.

1. Add the empty boxes.
2. Assign the lowest CPUE to all NA boxes (including boundary and islands)
3. Multiply CPUEs by areas.
4. Set boundary and islands to 0
5. Work out the proportions as index/sum(index)

```{r}
mincpue <- min(means_all_years$all_years_cpue, na.rm=T)

s <- atlantis_boxes %>%
  left_join(means_all_years %>% st_set_geometry(NULL) %>% select(box_id,all_years_cpue), by = 'box_id') %>%
  rowwise() %>%
  mutate(all_years_cpue = ifelse(isTRUE(boundary) | botz==0,
                                 0,
                                 ifelse(is.na(all_years_cpue),mincpue,all_years_cpue))) %>%
  ungroup() %>%
  mutate(index = all_years_cpue*area,
         prop = index/sum(index))

# have a look if you want
# s %>% ggplot()+geom_sf(aes(fill=prop))+scale_fill_viridis()+theme_bw()

# write that out
s %>% st_set_geometry(NULL) %>% select(.bx0,prop) %>% write.csv('s1s4_2.csv', row.names = FALSE)
```
