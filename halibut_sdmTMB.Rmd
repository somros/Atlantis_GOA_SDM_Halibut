---
title: "Adult Pacific halibut NPUE with sdmTMB - lat, lon, depth"
...
ggplot()+
  geom_point(data = halibut_data, aes(lon, lat, colour = log1p(npue)), size = 1.5)+
...
```{r}
biom_year <- halibut_data %>% group_by(year) %>% summarise(npue = sum(npue))
...
```{r}
hist(halibut_data$npue, breaks = 30)
```

```{r}
hist(log1p(halibut_data$npue), breaks = 30)
```

Proportion of zeroes in percentage.
```{r}
length(which(halibut_data$npue == 0))/nrow(halibut_data)*100
```
...
if(abs(max(m_depth$gradients))>0.001){
  ...
  m_depth <- sdmTMB(
    data = halibut_data, 
    formula = npue2 ~ 0 + s(depth, k = 3) + as.factor(year), 
    mesh = halibut_spde, 
    time = "year", 
    spatial = 'on',
    spatiotemporal = 'iid', # spatiotemporal random fields independent and identically distributed
...
```{r, fig.width = 12, fig.height = 18}
ggplot()+
  geom_sf(data = halibut_data_sf, aes(colour = log1p(npue)), size = 1.5, alpha = .5)+
...
```{r}
ggplot(data = halibut_data, aes(x = depth, y = log1p(npue)))+
  scale_color_viridis()+
  geom_point()+
  theme_minimal()+
...
```{r}
ggplot(data = halibut_data, aes(x = log1p(npue)))+
...
```{r}
paste0("Pearson's coef observations vs predictions: ", cor(halibut_corr$npue, halibut_corr$predictions, use = "everything", method = "pearson"))
```

Plot.
```{r, fig.width = 12, fig.height = 12}
ggplot(halibut_corr, aes(x = log1p(npue), y = log1p(predictions)))+ # log for visualisation
...
```{r}
paste("RMSE:", sqrt(sum((halibut_corr$predictions - halibut_corr$npue)^2)/nrow(halibut_corr)), " ind hook-1", sep = " ") ### traditional rmse metric, in units kg km2
```

Normalised RMSE. 
```{r}
rmse_cv <- sqrt(sum((halibut_corr$predictions - halibut_corr$npue)^2)/nrow(halibut_corr))/(max(halibut_corr$npue)-min(halibut_corr$npue))*100 #### normalised rmse, expressed as a % of the range of observed biomass values, sort of approximates a coefficient of variation 
...
The problem is that these are NPUEs in fish per hook. Multiplying these by areas makes no sense, but to obtain an abstract abundance index that then we use to work out relative proportions per box it should be OK.

1. Add the empty boxes.
2. Assign the lowest NPUE to all NA boxes (including boundary and islands)
3. Multiply NPUEs by areas.
4. Set boundary and islands to 0
5. Work out the proportions as index/sum(index)

```{r}
minnpue <- min(means_all_years$all_years_npue, na.rm=T)

s <- atlantis_boxes %>%
  left_join(means_all_years %>% st_set_geometry(NULL) %>% select(box_id,all_years_npue), by = 'box_id') %>%
  rowwise() %>%
  mutate(all_years_npue = ifelse(isTRUE(boundary) | botz==0,
                                 0,
                                 ifelse(is.na(all_years_npue),minnpue,all_years_npue))) %>%
  ungroup() %>%
  mutate(index = all_years_npue*area,
...
# write that out
s %>% st_set_geometry(NULL) %>% select(.bx0,prop) %>% write.csv('s1s4_1.csv', row.names = FALSE)
```
